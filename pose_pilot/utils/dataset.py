...
class MyCarotidDataset(Dataset):
    def __init__(self,
        ...
        for seq in seq_names:
            ...
            for i in range(0, end_i + 1, self.image_interval):
                ...
                # 2. 【关键修改】如果 YOLO 没看到血管，直接跳过
                # 因为我们要训练“对中策略”，没有血管就无法计算目标距离
                if ok != 1:
                    continue
                
                # 3. 添加原始图片样本 (Base Sample)
                # 即使是原图，后续在 __getitem__ 里也会被强制用 YOLO 计算“回中”标签
                # 这样利用了原图的真实性，同时修正了标签逻辑
                ...
                # 基于原图生成 K 个平移副本，进一步增强对位置偏移的鲁棒性
                if self.shift_mode == 'dynamic' and self.dynamic_k > 0:
    ...
    def __getitem__(self, index):
        ...
        # 4. 读取 YOLO 信息
        yb = meta['yolo']
        # 注意：在 __init__ 中我们已经保证了进入这里的样本基本都是 ok=1 的
        # 但为了安全，这里还是做个防御性读取
        ...
        if self.gt_policy == 'pixel_dis':
            ...
            # 如果 y_ok=0，保持原有的 dT_1e (虽然 __init__ 里过滤了，但做个兜底)

        elif self.gt_policy == 'keep_dt1f':
            pass 
        else:

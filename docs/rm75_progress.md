...
| Haptron Modbus 与补偿 wrench | 已实现，正式标定待完成 | v6 provisional 正在使用；v8 已采样，尚待使用已测量 R/t/TCP 完成拟合和验收。 |
...
1. 使用 `calibration_samples_v8.csv` 和独立实测的 R/t/TCP 完成正式标定，替换临时 v6 JSON，并用多姿态悬空残差验收。
2. 在已知表面测量接触点估计误差，并对比 Kalman 开/关的接触点与 Roll 轨迹。
3. 在软质仿体上单独验收：`-3 N` 稳态、超力卸载、制动、idle 不重接近与重新接触。
4. 依次验收 Tool-Y 正负方向、Tool-X 连续扫描、phase=1 的 RZ 符号和 120° 累计额度。

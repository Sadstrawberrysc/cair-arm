...
从零熟悉架构、源码和算法时，请按 [RM75 学习路径](rm75_learning_path.md) 的模块顺序阅读；
本文继续作为当前状态、运行边界和真机记录的索引。
...
但仍保留 `3.5 N / 0.2 N·m` 的姿态现场 tare 门。
...
| Tool-X 扫描 | 稳定力启动门已完成，真机待验收 | 接触后先保持 Z 导纳；`Fz` 连续 `0.5 s` 位于目标 `-3±0.8 N` 才锁存启动 `-Tool X`，无时间和扫描距离终点 |
| Y/RZ 修正与完整扫描闭环 | 软件接入，Y 居中真机复验中 | 视觉 Tool-Y 在接触、moving 且非卸载/恢复时立即生效，不再等待 `Fz≤-2 N/0.5 s`；Y 比例速度增益为 `1.0 s⁻¹`，实际跟踪暂停门由 `2/1 mm` 放宽为 `10/5 mm`，避免正常接触误差长期屏蔽颈动脉居中 |
...
3. 接触力连续 `0.5 s` 稳定在 `-3±0.8 N` 后，验证锁存启动 `-Tool X`
   持续运动，同时保持 Z 向力控。
...
| 扫描阶段 | `phase_index=0`；`|Fz|≥2 N` 为粗接触下限，且 `Fz` 必须连续 `0.5 s` 位于 `-3±0.8 N` 才启动 |
...
`|Fz-(-3 N)|≤0.8 N` 且 `|Fz|≥2 N` 时累积；任一周期越界都会重新计时。连续满足
...
`0.5 s` 稳定在 `-3±0.8 N`，再锁存并沿 `-Tool X` 持续运动，Z 向 `-3 N` 力控

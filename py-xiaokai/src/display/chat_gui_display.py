...
class GuiDisplay(BaseDisplay, QObject, metaclass=CombinedMeta):
    ...
    async def update_chat(self, messages):
        """
        更新聊天消息列表，用于 QML 聊天气泡视图.
        """
        try:
            self.display_model.set_chat_messages(messages)
    ...
    def _create_main_window(self):
        ...
        # 使用普通窗口样式，保留系统标题栏和边框，允许用户拖动边缘调整大小
        ...
        # 设置一个合理的最小尺寸，避免被缩得过小
        self.root.setMinimumSize(640, 400)
    ...
    def _restore_window_geometry(self):
        ...
        try:
            ...
            geom = config_manager.get_config("SYSTEM_OPTIONS.CHATGUI_WINDOW", None)
            ...
            # 一旦使用用户保存的几何信息，就不再按照全屏模式显示
            self._is_fullscreen = False

            # 恢复尺寸（确保不小于最小尺寸）
            if w > 0 and h > 0:
                min_w = max(self.root.minimumWidth(), 200)
                min_h = max(self.root.minimumHeight(), 150)
                self.root.resize(max(w, min_w), max(h, min_h))
            ...
            if screen_rect.width() > 0 and screen_rect.height() > 0:
                # 简单裁剪到屏幕范围内
                x = max(screen_rect.left(), min(x, screen_rect.right() - 50))
                y = max(screen_rect.top(), min(y, screen_rect.bottom() - 50))
            self.root.move(x, y)
        except Exception as e:
            self.logger.error(f"恢复窗口位置/大小失败: {e}", exc_info=True)
    ...
    def _save_window_geometry(self):
        ...
        try:
            ...
            data = {
                ...
                "width": int(g.width()),
                "height": int(g.height()),
                "maximized": bool(self.root.isMaximized()),
            }

            config_manager = ConfigManager.get_instance()
            config_manager.update_config("SYSTEM_OPTIONS.CHATGUI_WINDOW", data)
        except Exception as e:
            self.logger.error(f"保存窗口位置/大小失败: {e}", exc_info=True)
    ...
    def _load_qml(self):
        ...
        # 加载 QML 文件
        qml_file = Path(__file__).parent / "chat_gui_display.qml"
    ...
    async def _finalize_startup(self):
        ...
        elif getattr(self, "_start_maximized", False):

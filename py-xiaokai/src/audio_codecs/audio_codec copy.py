...
class AudioCodec:
    ...
    async def clear_audio_queue(self, *, max_age_seconds: int = 24 * 3600, dry_run: bool = False) -> int:
        import tempfile
        import time
        import os
        from pathlib import Path
        """
        清空音频队列，并删除系统临时目录（tmp）下最近修改的 mp3 文件。
        参数:
        max_age_seconds: 仅删除在此时间范围内被修改的 mp3（默认 24 小时）。若为 None 则不按时间过滤（小心使用）。
        dry_run: 如果为 True，只记录将删除哪些文件，但不实际删除（测试用）。
        返回:
        cleared_count: 被清理的“帧/缓冲/文件”总数量
        """
        cleared_count = 0

        # ========== 清空队列（保持并改进原有逻辑） ==========
        queues_to_clear = [
            getattr(self, "_wakeword_buffer", None),
            getattr(self, "_output_buffer", None),
        ]

        for queue in queues_to_clear:
            if queue is None:
                continue
            # 如果 queue 支持 empty() 与 get_nowait()
            while True:
                try:
                    # 优先使用非阻塞的 get_nowait()
                    if hasattr(queue, "get_nowait"):
                        ...
                        # 若没有 get_nowait，尝试调用 synchronous pop/clear if possible
                        if hasattr(queue, "clear"):
                            try:
                                # 若是 deque/list-like
                                length_before = len(queue)
                                queue.clear()
                                cleared_count += length_before
                            except Exception:
                                pass
                        break
                except asyncio.QueueEmpty:
        ...
        try:
            ...
            if p.exists() and p.is_dir():
                for mp3_path in p.glob("*.mp3"):  # 非递归，仅当前 tmp 目录
                    ...
                    try:
                        ...
                        if max_age_seconds is not None:
                            ...
                            if age > max_age_seconds:
                                ...
                                continue
                        if dry_run:
                            logger.info(f"[dry_run] 将删除 tmp 下 mp3: {mp3_path}")
                            mp3_deleted += 1
                        else:
                            try:
                                mp3_path.unlink()
                                mp3_deleted += 1
                                logger.debug(f"删除 tmp 下 mp3 文件: {mp3_path}")
                            except Exception as e:
                                logger.warning(f"删除 mp3 文件失败: {mp3_path}，原因: {e}")
                    except Exception as e:
                        logger.debug(f"处理 mp3 文件 {mp3_path} 时出错: {e}")
        except Exception as e:
            logger.debug(f"扫描 tmp 目录删除 mp3 时发生错误: {e}")

        cleared_count += mp3_deleted

        # 日志与 GC（保留原逻辑）
        if cleared_count > 0:
            logger.info(f"清空音频队列，丢弃 {cleared_count} 帧音频数据（其中 {mp3_deleted} 个为 tmp 下的 mp3 文件）")

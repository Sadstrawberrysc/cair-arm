...
class AudioWidget(QWidget):
    ...
    def _do_input_test(self, device_id):
        ...
        try:
            ...
            # --- 关键修复：暂时停止可能占用设备的全局 AudioCodec ---
            from src.application import Application
            app = Application.get_instance()
            codec_paused = False
            
            try:
                # 检查 audio_codec 是否存在且正在运行
                if hasattr(app, "audio_codec") and app.audio_codec:
                    self.logger.info("检测到主程序 AudioCodec，尝试挂起以释放设备锁...")
                    future = asyncio.run_coroutine_threadsafe(
                        app.audio_codec.stop_streams(), 
                        app._main_loop
                    )
                    future.result(timeout=2)  # 等待停止完成
                    codec_paused = True
                    time.sleep(0.5) # 稍微等待驱动释放
            except Exception as e:
                self.logger.warning(f"尝试挂起 AudioCodec 失败: {e}")
            ...
            try:
                ...
                # 如果失败，且原采样率不是16000，则尝试降级到16000Hz
                if sample_rate != 16000:
                    self.logger.warning(f"默认采样率 {sample_rate}Hz 录音失败: {e1}，尝试使用 16000Hz 重试...")
                    self._append_status_threadsafe(f"默认采样率失败，尝试使用 16000Hz 重试...")
                    try:
                        sample_rate = 16000
                        recording = sd.rec(
                            int(duration * sample_rate),
                        ...
                        self.logger.error(f"16000Hz 重试也失败: {e2}")
                        raise e1  # 抛出原始异常或e2
                else:
                    raise e1
            ...
            if codec_paused:
                try:
                    self.logger.info("测试完成，恢复主程序 AudioCodec...")
                    asyncio.run_coroutine_threadsafe(
                        app.audio_codec.start_streams(), 
                        app._main_loop
                    )
                except Exception as e:
                    self.logger.warning(f"恢复 AudioCodec 失败: {e}")

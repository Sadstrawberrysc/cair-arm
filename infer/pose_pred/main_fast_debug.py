...
def main():
    ...
    posehead_ckpt = '/home/cair-jacen/embodiedUS/IntegratedSystem_Jiuan/Carotid/ModelV2/weights_old/pose/bestV2.pth'   # best_pixel_ph4
            ...
            if trigger_inference:
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=torch.cuda.is_available()):
                        ...
                        if(lock_flag == 0):
                            if(abs(y_value) < lock_band ):
                                near_zero_count += 1
                            else:
                                near_zero_count = 0
                        
                            if(near_zero_count >= count_to_lock):
                                lock_flag = 1
                                big_err_count = 0
                        
                        if(lock_flag == 1):
                            print("Locked")
                            if(abs(y_value) > unlock_band):
                                big_err_count += 1
                            else:
                                big_err_count = 0
                        
                            if(big_err_count >= count_to_unlock):
                                lock_flag = 0
                                near_zero_count = 0
                                big_err_count = 0
                            else:

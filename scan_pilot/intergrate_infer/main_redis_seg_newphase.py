...
def main():
    ...
    # PhaseHead loading logic
    if os.path.isfile(phasehead_ckpt):
        print(f"Loading PhaseHead weights from: {phasehead_ckpt}")
        try:
            ckpt = torch.load(phasehead_ckpt, map_location='cpu')
        except Exception as e:
            if 'argparse.Namespace' in str(e):
                ...
                try:
                    ...
                    ckpt = torch.load(phasehead_ckpt, map_location='cpu', weights_only=False)
            else:
                ckpt = torch.load(phasehead_ckpt, map_location='cpu', weights_only=False)
        ...
        msg = phasehead.load_state_dict(clean_state_dict, strict=True)
        print(f"phasehead loaded: missing={len(msg.missing_keys)} unexpected={len(msg.unexpected_keys)}")
    else:
        print(f"phasehead ckpt not found: {phasehead_ckpt}")
